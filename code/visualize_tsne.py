...
checkpoint = torch.load("/Data/summer24/DreamDiffusion/DreamDiffuion/results/eeg_pretrain/23-07-2024-09-02-15/checkpoints/checkpoint.pth", map_location='cpu')  # Modify the path to your checkpoint
model.load_state_dict(checkpoint['model'], strict=False)
model.eval()
all_latents = []
import pandas as pd
df = pd.read_csv(f"/Data/summer24/DreamDiffusion/datasets/eegdata/{mode}/{type}.csv",index_col=0)
...
plt.colorbar()
plt.show()
plt.savefig(f'/Data/summer24/DreamDiffusion/result/23-07_checkpoint_tsne_{mode}_{type}.png')